...
def Test():
    ...
    # # # #System parameters:
    # args.Model='Voter' # type of models: 'No','FA', 'East', see args.py for a the help information
    # args.L=6# Lattice size: 1D
    # args.Tstep=11#Time step of iterating the dynamical equation P_tnew=T*P_t, where T=(I+W*delta t)
    # args.delta_t=0.05#0.1#Time step length
    ...
    # args.max_stepAll=30#0#0 #The epoch for the 1st  time steps
    # args.max_stepLater=5#0#00 #The epoch at time steps except for the 1st step
    ...
    # args.batch_size=100#00#1000 #batch size
    # args.BC=3
    ...
    args.size = args.L * args.L  # the number of spin: doesnt' count the boundary spins
    args.epsilon = 0  # 1e-300#0#1 # avoid 0 value in log below
    ...
    for lambda_tilt in lambda_tilt_Range:
        ...
        for Tstep in range(startT, args.Tstep):  # Time step of the dynamical equation
            ...
            with torch.no_grad():
                ...
                if Tstep % int(args.print_step) == 0:  # Save samples only every print_step steps
                    SampleSum.append(np.array(SampleT).reshape(-1, args.L, args.L))
                    argsSave = [
                        args.Tstep,
                        args.delta_t,
                        args.L,
                    ...
                    np.savez(
                        '{}_img/Data{}L{}c{}s{}'.format(
                            args.out_filename, args.Model, args.L, args.c, args.dlambdaL
                        ),
            ...
            # #Plot training loss
            if Tstep <= 1 or Tstep == 500 or Tstep == 1000:
                ...
                axes.set_yscale('log')
                plt.plot(
                    range(0, args.max_step + 1), np.abs(ListDistanceCheck_Eucli), label='Euclidean distance'
                )  # ,, label = 'Relative mean square error')
                plt.plot(
        ...
        # np.savez('{}_img/DataL{}c{}s{}'.format(args.out_filename,args.L,args.c,args.dlambdaL),np.array(SummaryListDynPartiFuncLog2),np.array(SummaryListDynPartiFuncLog3),np.array(SummaryLoss1),np.array(SummaryLoss2))
        argsSave = [
            args.Tstep,
            args.delta_t,
            args.L,
        ...
        # np.savez('{}_img/DataL{}c{}s{}'.format(args.out_filename,args.L,args.c,args.dlambdaL),np.array(SummaryListDynPartiFuncLog2),argsSave,np.array(SummaryListDynPartiFuncLog3),np.array(SummaryLoss1),np.array(SummaryLoss2))
        np.savez(
    ...
    if args.lossType == 'ss':
        np.savez(
            '{}_img/Data{}SSL{}c{}s{}'.format(args.out_filename, args.Model, args.L, args.c, args.dlambdaL),
            SummaryListDynPartiFuncLog2,
            argsSave,
            SummaryListDynPartiFuncLog3,
            SummaryLoss1,
