...
def get_ham_args_features():
    ...
    ham_args = '{Model}_L{L}_T{Tstep}_dt{delta_t}_batch{batch_size}_c{c}'
    ...
    if args.net == 'made':
        ...
        features = 'nd{net_depth}_nw{net_width}_stackedpixelcnn'
    elif args.net == 'lstm':
        features = 'nd{net_depth}_nw{net_width}_lstm'
    else:
        features = 'nd{net_depth}_nw{net_width}_hks{half_kernel_size}'

    features += '_lr{lr}_sL{dlambdaL}_sR{dlambdaR}_ds{dlambda}'
    features += '_lepoch{max_stepLater}'
    ...
    if args.neighborS:
        features += '_neighborS'
    if args.addBatch:
        features += '_addBatch{Number2}'
    if args.IS:
        features += '_IS{ISNumber1}_p{binomialP}_OnT{max_stepT1}'
    if not args.SwitchOffIS:
        features += '_NotOff'
    if args.reverse:
        features += '_WithRev'
    if args.z2:
    ...
    if args.lr_schedule:
        features += '_lrs{lr_schedule_type}'
    if args.negativeS:
        features += '_negativeS'
    if args.Hermitian:
        features += '_Hermitian'
