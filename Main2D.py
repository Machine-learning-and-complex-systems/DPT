...
def Test():
    ...
    args.batch_size = 1  # 00#1000 #batch size
    ...
    if args.size <= 16:
        ...
        Sample2D = Sample1DExtend.view(-1, args.L, args.L)  # Correct: sample has size batchsize X L X L
        # Correct: The connected states flip into this sampled state
        Win = (Sample1D - 1).abs() * (1 - args.c) + Sample1D * args.c
        Col1 = torch.cat((torch.zeros(Sample2D.shape[0], args.L, 1), Sample2D[:, :, :-1]), 2)  # down sites
        Col2 = torch.cat((Sample2D[:, :, 1:], torch.zeros(Sample2D.shape[0], args.L, 1)), 2)
        Row1 = torch.cat((torch.zeros(Sample2D.shape[0], 1, args.L), Sample2D[:, :-1, :]), 1)
        Row2 = torch.cat((Sample2D[:, 1:, :], torch.zeros(Sample2D.shape[0], 1, args.L)), 1)
        if args.BC == 1:
    ...
    for lambda_tilt in lambda_tilt_Range:
        ...
        for Tstep in range(startT, args.Tstep):  # Time step of the dynamical equation
            # This is to get numerical exact result for small systems
            if args.size <= 16:  # compare with numerical exact result
                if Tstep == 0:
        ...
        # This is to compare with numerical exact result for small systems
        if args.size <= 16:
            ...
            plt.plot(
                np.arange(len(ListDynPartiFuncLog2_3)) + 1, DeltaLogZ2_3, 'kx', markersize=8, label='VAN'
            )  # ,, label = 'Relative mean square error')
        ...
        # if args.size<=16:
        #     plt.plot(np.arange(len(ListDynPartiFuncLog2_3))+1, ListDynPartiFuncLog3/args.size, 'b',linewidth=0.8,label='Numerical exact')#,, label = 'Relative mean square error')
        # plt.xlim((1, len(ListDynPartiFuncLog2_3)))
        # axes = plt.gca()
        # axes.set_xscale('log')
        # axes.set_yscale('log')
